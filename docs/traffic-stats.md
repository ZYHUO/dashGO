# 流量统计功能说明

## 功能概述

dashGO 提供了完整的流量统计功能，用于监控和分析系统的流量使用情况。

## 数据库表结构

### 1. v2_stat_user - 用户流量统计表

记录每个用户的流量使用情况。

| 字段 | 类型 | 说明 |
|------|------|------|
| id | bigint | 主键 |
| user_id | bigint | 用户ID |
| server_rate | decimal(10,2) | 服务器倍率 |
| u | bigint | 上传流量（字节） |
| d | bigint | 下载流量（字节） |
| record_type | varchar(2) | 记录类型（d=日，m=月） |
| record_at | bigint | 记录时间戳 |
| created_at | bigint | 创建时间 |
| updated_at | bigint | 更新时间 |

### 2. v2_stat_server - 节点流量统计表

记录每个节点的流量使用情况。

| 字段 | 类型 | 说明 |
|------|------|------|
| id | bigint | 主键 |
| server_id | bigint | 节点ID |
| server_type | varchar(11) | 节点类型 |
| u | bigint | 上传流量（字节） |
| d | bigint | 下载流量（字节） |
| record_type | varchar(1) | 记录类型（d=日，m=月） |
| record_at | bigint | 记录时间戳 |
| created_at | bigint | 创建时间 |
| updated_at | bigint | 更新时间 |

### 3. v2_stat - 全局统计表

记录系统的全局统计数据。

| 字段 | 类型 | 说明 |
|------|------|------|
| id | bigint | 主键 |
| record_at | bigint | 记录时间戳（唯一） |
| record_type | varchar(1) | 记录类型（d=日，m=月） |
| order_count | int | 订单数量 |
| order_total | bigint | 订单总额 |
| commission_count | int | 佣金订单数 |
| commission_total | bigint | 佣金总额 |
| paid_count | int | 已支付订单数 |
| paid_total | bigint | 已支付总额 |
| register_count | int | 注册用户数 |
| invite_count | int | 邀请用户数 |
| transfer_used_total | varchar(32) | 总流量使用 |
| created_at | bigint | 创建时间 |
| updated_at | bigint | 更新时间 |

### 4. v2_server_log - 节点流量日志表

记录详细的流量日志。

| 字段 | 类型 | 说明 |
|------|------|------|
| id | bigint | 主键 |
| user_id | bigint | 用户ID |
| server_id | bigint | 节点ID |
| u | bigint | 上传流量（字节） |
| d | bigint | 下载流量（字节） |
| rate | decimal(10,2) | 倍率 |
| method | varchar(32) | 协议方法 |
| created_at | bigint | 创建时间 |
| updated_at | bigint | 更新时间 |

## API 接口

### 流量统计接口

#### 1. 流量概览

```
GET /api/v2/admin/traffic/overview
```

返回数据：
```json
{
  "total_upload": 1073741824,
  "total_download": 5368709120,
  "total_traffic": 6442450944,
  "active_users": 10,
  "top_users": [
    {
      "user_id": 1,
      "email": "user@example.com",
      "upload": 536870912,
      "download": 2684354560,
      "total": 3221225472
    }
  ],
  "upload_percent": 16.67,
  "download_percent": 83.33
}
```

#### 2. 节点流量统计

```
GET /api/v2/admin/traffic/servers
```

返回数据：
```json
[
  {
    "server_id": 1,
    "server_name": "香港节点",
    "server_type": "shadowsocks",
    "upload": 536870912,
    "download": 2684354560,
    "total": 3221225472
  }
]
```

#### 3. 每日流量统计

```
GET /api/v2/admin/traffic/daily?days=30
```

返回最近 N 天的流量统计数据。

#### 4. 用户流量详情

```
GET /api/v2/admin/traffic/user/:id
```

返回指定用户的详细流量信息。

### 流量管理接口

#### 5. 获取流量统计

```
GET /api/v2/admin/traffic/stats
```

返回流量统计概览，包括总流量、活跃用户数、超流量用户数等。

#### 6. 获取流量预警用户

```
GET /api/v2/admin/traffic/warnings?threshold=80
```

参数：
- `threshold`: 流量使用阈值（百分比），默认 80

返回流量使用超过阈值的用户列表。

#### 7. 重置用户流量

```
POST /api/v2/admin/traffic/reset/:id
```

重置指定用户的流量（将 u 和 d 设置为 0）。

#### 8. 批量重置流量

```
POST /api/v2/admin/traffic/reset-all
```

重置所有需要重置流量的用户。

#### 9. 发送流量预警通知

```
POST /api/v2/admin/traffic/warning/:id
```

向指定用户发送流量预警通知（邮件）。

#### 10. 批量发送流量预警

```
POST /api/v2/admin/traffic/warnings/send?threshold=80
```

向所有流量使用超过阈值的用户批量发送预警通知。

#### 11. 自动封禁超流量用户

```
POST /api/v2/admin/traffic/autoban
```

自动封禁所有流量超限的用户。

### Agent 接口

#### 12. 上报流量

```
POST /api/v1/agent/traffic
```

Agent 上报流量数据。

请求体：
```json
{
  "nodes": [
    {
      "id": 1,
      "users": [
        {
          "username": "user-uuid",
          "upload": 1024000,
          "download": 2048000
        }
      ]
    }
  ]
}
```

## 前端页面

### 流量统计页面

路径：`/admin/traffic`

功能：
- 📊 总流量、上传、下载统计卡片
- 📈 上传/下载占比饼图
- 📊 节点流量占比图表
- 📋 用户流量排行榜

### 流量管理页面

路径：`/admin/traffic-management`

功能：
- 📊 流量统计概览
- ⚠️ 流量预警用户列表
- 🔄 批量重置流量
- 📧 批量发送预警通知
- 🚫 自动封禁超流量用户

## 流量记录机制

### 1. 实时记录

当 Agent 上报流量数据时，系统会：
1. 更新用户的 `u`（上传）和 `d`（下载）字段
2. 记录到 `v2_server_log` 表（详细日志）
3. 更新 `v2_stat_user` 统计表（用户维度）
4. 更新 `v2_stat_server` 统计表（节点维度）

**实现细节**：
- Agent 每分钟上报一次流量增量
- 优先使用用户级流量统计（通过 Clash API）
- 备用方案：端口流量平均分配
- 所有流量数据都应用节点倍率

### 2. 统计周期

- **实时统计**：Agent 每分钟上报，立即更新数据库
- **日统计（d）**：每天 00:00 自动生成前一天的统计数据
- **月统计（m）**：每月 1 日 00:00 自动生成上月的统计数据

### 3. 倍率计算

流量会根据节点的倍率（rate）进行计算：

```
实际扣除流量 = (上传 + 下载) × 倍率
```

例如：
- 节点倍率为 1.0：使用 1GB 扣除 1GB
- 节点倍率为 0.5：使用 1GB 扣除 0.5GB
- 节点倍率为 2.0：使用 1GB 扣除 2GB

### 4. 数据清理

系统会自动清理旧数据：
- **流量日志**：保留 90 天，每周一自动清理
- **日统计**：保留 1 年，每周一自动清理
- **月统计**：永久保留

## 注意事项

### ⚠️ 流量统计精度

当前实现使用**平均分配算法**，单用户流量统计可能不够精确。这是因为：

1. **多用户共享节点**：多个用户同时使用同一个节点时，流量是平均分配的
2. **无精确流控**：系统不支持精确的单用户流量控制
3. **统计延迟**：流量数据由 Agent 定期上报，存在一定延迟

### 适用场景

- ✅ 监控总体流量趋势
- ✅ 识别高流量用户
- ✅ 节点流量分析
- ✅ 流量预警和管理
- ❌ 精确的单用户计费（需要专业流控方案）

### 改进建议

如需精确的单用户流量统计，建议：

1. **使用专业流控方案**
   - 在节点上实现真正的多用户流控
   - 使用 iptables/nftables 进行精确统计

2. **独立节点部署**
   - 为每个用户分配独立的节点端口
   - 通过端口级别统计流量

3. **使用第三方流控插件**
   - 集成专业的流量统计插件
   - 实现更精确的流量记录

## 部署和配置

### 1. 数据库迁移

创建流量统计表：

```bash
# 执行迁移
./migrate-linux-amd64 -config configs/config.yaml

# 或使用 Docker
docker exec dashgo ./migrate -config /app/configs/config.yaml
```

迁移文件位置：
- `migrations/005_create_traffic_stats_tables.sql`
- `migrations/005_create_traffic_stats_tables_rollback.sql`

### 2. Agent 配置

Agent 会自动上报流量数据，无需额外配置。确保：

1. Agent 正常运行并连接到面板
2. sing-box 配置中启用了 Clash API
3. Agent 每分钟自动上报流量

### 3. 定时任务

系统会自动运行以下定时任务：

- **每分钟**：Agent 上报流量
- **每小时**：发送流量预警通知
- **每天凌晨**：生成日统计、发送到期提醒
- **每月 1 号**：重置月流量、生成月统计
- **每周一**：清理旧数据

### 4. 邮件配置

如需使用流量预警通知功能，请配置邮件服务：

```yaml
mail:
  host: smtp.example.com
  port: 587
  username: your-email@example.com
  password: your-password
  from: noreply@example.com
```

## 性能优化

### 1. 索引优化

已创建的索引：
- `user_id` - 用户查询
- `server_id` - 节点查询
- `record_at` - 时间范围查询
- `created_at` - 日志查询

### 2. 数据清理

建议定期清理旧的流量日志：

```sql
-- 删除 90 天前的流量日志
DELETE FROM v2_server_log WHERE created_at < UNIX_TIMESTAMP(DATE_SUB(NOW(), INTERVAL 90 DAY));

-- 删除 1 年前的日统计
DELETE FROM v2_stat_user WHERE record_type = 'd' AND record_at < UNIX_TIMESTAMP(DATE_SUB(NOW(), INTERVAL 1 YEAR));
DELETE FROM v2_stat_server WHERE record_type = 'd' AND record_at < UNIX_TIMESTAMP(DATE_SUB(NOW(), INTERVAL 1 YEAR));
```

### 3. 分表策略

对于大流量系统，建议：
- 按月分表存储流量日志
- 使用时间序列数据库（如 InfluxDB）存储流量数据
- 定期归档历史数据

## 故障排查

### 流量统计不准确

1. 检查 Agent 是否正常上报
2. 检查节点倍率配置
3. 查看 `v2_server_log` 表是否有记录
4. 检查定时任务是否正常运行

### 流量数据丢失

1. 检查数据库连接
2. 查看系统日志
3. 确认 Agent 心跳正常
4. 检查数据库磁盘空间

### 性能问题

1. 检查数据库索引
2. 优化查询语句
3. 增加数据库连接池
4. 考虑使用缓存（Redis）

## 相关文档

- [Agent 自动更新](agent-auto-update.md)
- [主机管理](../README.md#主机管理)
- [节点管理](../README.md#节点管理)
