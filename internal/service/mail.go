...
func (s *MailService) SendVerifyCode(to, code string) error {
	...
	body := fmt.Sprintf(`
		<div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 600px; margin: 0 auto; padding: 40px 20px;">
			...
			<p style="color: #999; font-size: 14px; margin-top: 20px;">验证码有效期告10 分钟，请勿泄露给他人告/p>
		</div>
	`, code)
...
func (s *MailService) SendWelcome(user *model.User) error {
	...
	body := fmt.Sprintf(`
		<div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 600px; margin: 0 auto; padding: 40px 20px;">
			...
			<p style="color: #666; font-size: 16px; line-height: 1.6;">您的账号告strong>%s</strong></p>
			<div style="margin-top: 30px;">
				<a href="#" style="background: linear-gradient(135deg, #667eea 0%%, #764ba2 100%%); color: white; padding: 12px 30px; border-radius: 8px; text-decoration: none; font-weight: 500;">开始使告/a>
			</div>
		</div>
	`, user.Email)
...
// SendExpireReminder 发送到期提告
func (s *MailService) SendExpireReminder(user *model.User, daysLeft int) error {
	subject := "订阅即将到期提醒"
	body := fmt.Sprintf(`
		<div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 600px; margin: 0 auto; padding: 40px 20px;">
			<h2 style="color: #1a1a2e; margin-bottom: 20px;">告订阅即将到期</h2>
			<p style="color: #666; font-size: 16px; line-height: 1.6;">您好，您的订阅将告<strong style="color: #e74c3c;">%d 告/strong>后到期告/p>
			<p style="color: #666; font-size: 16px; line-height: 1.6;">为避免服务中断，请及时续费告/p>
			<div style="margin-top: 30px;">
...
func (s *MailService) SendTrafficWarning(user *model.User, usedPercent int) error {
	...
	body := fmt.Sprintf(`
		<div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 600px; margin: 0 auto; padding: 40px 20px;">
			...
			<p style="color: #666; font-size: 16px; line-height: 1.6;">您好，您的流量已使用 <strong style="color: #e74c3c;">%d%%</strong>告/p>
			...
			<p style="color: #999; font-size: 14px;">请合理使用流量，或考虑升级套餐告/p>
		</div>
...
func (s *MailService) SendOrderPaid(user *model.User, order *model.Order) error {
	...
	body := fmt.Sprintf(`
		<div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 600px; margin: 0 auto; padding: 40px 20px;">
			<h2 style="color: #1a1a2e; margin-bottom: 20px;">告支付成功</h2>
			...
			<div style="background: #f8f9fa; border-radius: 12px; padding: 20px; margin: 20px 0;">
				...
				<p style="margin: 8px 0; color: #666;"><span style="color: #999;">金额告/span>¥%.2f</p>
			</div>
			<p style="color: #999; font-size: 14px;">感谢您的支持告/p>
		</div>
